        ...
        for i4 in range(len5):
            ...
            for j2 in range(len4):
                if (label1[j2] == label2[i4]) and (count == 0):
                    lname = '类别' + str(i4 + 1)
                    plt.scatter(linelabel, E4[j2], s=E4[j2] / 300000 + 10, label=lname, color=color1[i4], linewidth=1.0,
                                alpha=0.4)
                    count = count + 1
                if (label1[j2] == label2[i4]) and (count > 0):
                    plt.scatter(linelabel, E4[j2], s=E4[j2] / 300000 + 10, color=color1[i4], linewidth=1.0, alpha=0.4)

        plt.legend(loc='upper right', markerscale=0.7, labelspacing=1)
