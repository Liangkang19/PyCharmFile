# coding=utf-8
# 对全行业的原始数据进行聚类/清洗全零行，进行MiniBatchKMeans聚类
# 将聚类结果中位数等结果，进行可视化显示/散点图/条形图/折线图
...
for i in count:
    ...
    for j in year1:
        ...
        E1 = B[a:b, x:y]
        E2 = DataFrame(E1)
        D1 = E2.loc[~(E2 == 0).all(axis=1), :]  # 删除全零行
        E3 = np.array(D1)
        ...
        for i4 in range(len5):
            ...
            for j2 in range(len4):
                if (label1[j2] == label2[i4]) and (count == 0):
                    plt.scatter(linelabel, E3[j2], color=color1[i4], linewidth=1.0, label='类别' + str(i4 + 1))
                    count = count + 1
                if (label1[j2] == label2[i4]) and (count > 0):
                    plt.scatter(linelabel, E3[j2], color=color1[i4], linewidth=1.0)
