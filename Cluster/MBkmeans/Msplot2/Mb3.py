# coding=utf-8
# 使用小均值对全行业的原始数据进行聚类/清除全零行
# 将聚类结果中位数等结果，进行可视化显示/散点图
    ...
    for j in year1:
        ...
        for i4 in range(len4):
            if label1[i4] == 0:
                countA = countA + 1
                if countA == 1:
                    plt.scatter(linelabel, E3[i4], color='r', linewidth=1.0, label='类别1')
                else:
                    plt.scatter(linelabel, E3[i4], color='r', linewidth=1.0)
            if label1[i4] == 1:
                countB = countB + 1
                if countB == 1:
                    plt.scatter(linelabel, E3[i4], color='b', linewidth=1.0, label='类别2')
                else:
                    plt.scatter(linelabel, E3[i4], color='b', linewidth=1.0)
            if label1[i4] == 2:
                countC = countC + 1
                if countC == 1:
                    plt.scatter(linelabel, E3[i4], color='y', linewidth=1.0, label='类别3')
                else:
                    plt.scatter(linelabel, E3[i4], color='y', linewidth=1.0)
            if label1[i4] == 3:
                countD = countD + 1
                if countD == 1:
                    plt.scatter(linelabel, E3[i4], color='g', linewidth=1.0, label='类别4')
                else:
                    plt.scatter(linelabel, E3[i4], color='g', linewidth=1.0)
        plt.legend(loc='best')
        title4 = j + '企业月用电量聚类图2'
        plt.title(title4)
        plt.ylabel('用电量/千瓦时')
        plt.xlabel('月份')
        name4 = 'X' + temp[t] + '行业' + j + '企业聚类4'
        plt.savefig(name4)
        plt.close()
