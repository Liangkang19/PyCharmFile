# 绘制聚类中心条形图/聚类中心中位数折线图

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt

P1 = pd.read_excel(r'D:\Program Files\JetBrains\PyCharmFile\Matplot\f1.xlsx', sheet_name="Sheet1", header=0)
...
for i in range(len1):
    if i == (len1-1):
        plt.plot(ind, M3[i], color=color1[i], label='聚类中位数', linestyle='--', marker='^')
    else:
        plt.bar(ind + dis[i], M3[i], width, color=color1[i], label='类别' + str(i + 1))
