...
plt.figure()
for i in range(len1):
    count = 0                  # 在scatter()中增加：s=P3[j]/50000 即可改变散点大小
    for j in range(len2):
        if (P5[j] == P6[i]) and (count == 0):
            lname = '类别' + str(i+1)
            plt.scatter(linelabel, P3[j], color=color1[i], linewidth=1.0, label=lname, alpha=0.5)
            count = count + 1
        if (P5[j] == P6[i]) and (count > 0):
            plt.scatter(linelabel, P3[j], color=color1[i], linewidth=1.0,  alpha=0.5)
